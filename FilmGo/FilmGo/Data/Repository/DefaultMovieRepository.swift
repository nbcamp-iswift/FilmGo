        ...
        return Single.zip(detailReq, creditReq)
            .flatMap { detailDto, creditDto -> Single<Movie> in
                ...
                return self.networkService.downloadImage(from: posterURL)
                    .map { posterData in
                        let genres = detailDto.genres.map { $0.name }
                        ...
                        let director: String = {
                            creditDto.crew.first(where: { $0.job == "Director"})?.name ?? "Unknown"
                        }()

                        let actors: [String] = creditDto.cast.prefix(3).map { $0.name }
                        let runningTime = "\(detailDto.runtime)"
                        let voteAverage = (detailDto.voteAverage * 10).rounded() * 0.1;

                        return Movie(movieId: detailDto.id,
                              posterImage: posterData,
                              title: detailDto.title,
                              star: voteAverage,
                              runningTime: String(detailDto.runtime),
            ...
            .flatMap{ res in
                let movieSingles: [Single<MovieBrief>] = res.results
                    .map { summary in
                        ...
                        let posterReq: Single<Data> = self.networkService.downloadImage(from: posterURL)
